...
if ( ! function_exists( 'bb_theme_reply_link_attribute_change' ) ) {
    ...
    function bb_theme_reply_link_attribute_change( $retval, $r, $args ) {
	    ...
	    $r['uri'] = $uri;
	    $retval   = $r['link_before'] . '<a data-balloon=" ' . esc_html__( 'Reply', 'buddyboss-theme' ) . ' " data-balloon-pos="up" href="' . esc_url( $r['uri'] ) . '" class="bbp-reply-to-link"' . $modal . $onclick . '><i class="bb-icon-reply"></i><span class="bb-forum-reply-text">' . esc_html( $r['reply_text'] ) . '</span></a>' . $r['link_after'];
